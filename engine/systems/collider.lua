...
local function checkRectangleCollisions(x1, y1, x2, y2, box1, box2, velocity1,
  ...
  if box2.height > 0 then  -- not a cloud
    ...
    -- Diagonals
    if right1 <= left2 and bottom1 <= top2 and right1 + velocity1.x*dt > left2
        and bottom1 + velocity1.y*dt > top2 then
      velocity1.x = 0
      velocity1.y = 0
      position1.x = left2 - box1.width + box1.origin.x
      position1.y = top2 - box1.height + box1.origin.y
    end
    if left1 >= right2 and bottom1 <= top2 and left1 + velocity1.x*dt < right2
        and bottom1 + velocity1.y*dt > top2 then
      velocity1.x = 0
      velocity1.y = 0
      position1.x = right2 + box1.origin.x
    ...
    if right1 <= left2 and top1 >= bottom2 and right1 + velocity1.x*dt > left2
        and top1 + velocity1.y*dt < bottom2 then
      velocity1.x = 0
      velocity1.y = 0
      position1.x = left2 - box1.width + box1.origin.x
      position1.y = bottom2 + box1.origin.y
    end
    if left1 >= right2 and top1 >= bottom2 and left1 + velocity1.x*dt < right2
        and top1 + velocity1.y*dt < bottom2 then
      velocity1.x = 0
      velocity1.y = 0
      position1.x = right2 + box1.origin.x
